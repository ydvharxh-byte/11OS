...
// Environment Guard: If executed in Node.js (e.g. Vercel serverless auto-detection), safely bridge to server.js
...
// Local Persistence Engine (Guarantees zero data loss across Render sleeps & Vercel cold starts)
const BACKUP_STORAGE_KEY = 'study_os_student_backup_v1';

async function syncLocalBackup() {
  try {
    const backup = await api('/api/sync/backup');
    if (backup && backup.user) {
      localStorage.setItem(BACKUP_STORAGE_KEY, JSON.stringify(backup));
    }
  } catch (e) {
    console.warn('Local backup sync error:', e);
  }
}
...
async function restoreFromLocalBackup(backupData) {
  const payload = backupData || getLocalBackup();
  if (!payload) return false;
  try {
    const res = await api('/api/sync/restore', {
      method: 'POST',
      body: JSON.stringify(payload)
    });
    if (res?.ok) {
      toast('✦ Saved study state restored from local backup');
      return true;
    }
  } catch (e) {
    console.warn('Could not restore from local backup:', e);
  }
  return false;
}
...
function renderSettings() {
  ...
  $('#settings-view').innerHTML = `
    ...
    <!-- Data Persistence & Auto-Backup -->
    <div class="panel" style="max-width:760px;margin-bottom:18px">
      <div style="display:flex;justify-content:space-between;align-items:flex-start;flex-wrap:wrap;gap:10px">
        <div>
          <h3 style="margin-top:0;display:flex;align-items:center;gap:8px">
            <span>🛡️</span> Data Persistence &amp; Backup
          </h3>
          <p style="font-size:12px;color:var(--muted);margin-bottom:12px">
            Protects your Class 11 study records, notebook items, and scores against server spin-downs.
          </p>
        </div>
        <span class="tag tag-study" style="display:flex;align-items:center;gap:6px;padding:5px 10px">
          <span style="display:inline-block;width:6px;height:6px;border-radius:50%;background:#10b981"></span>
          AUTO-SYNC ACTIVE
        </span>
      </div>

      <div style="background:#f8f9fa;border:1px solid #e5e7eb;border-radius:12px;padding:12px 14px;margin-bottom:14px;font-size:12px;line-height:1.5;color:#374151">
        <strong>Automatic Protection:</strong> Your progress is continuously mirrored in your browser's persistent storage. Whenever Vercel or Render wakes up from sleep with a fresh container, your saved data is automatically restored in under 100ms.
      </div>

      <div style="display:flex;gap:10px;flex-wrap:wrap">
        <button type="button" class="primary" id="btn-download-backup" style="font-size:11px;padding:8px 14px">
          ⬇ Download Backup (.json)
        </button>
        <button type="button" class="outline" id="btn-trigger-upload-backup" style="font-size:11px;padding:8px 14px">
          ⬆ Restore from Backup (.json)
        </button>
        <input type="file" id="input-upload-backup" accept=".json" style="display:none" />
      </div>
    </div>
...
async function openModal(kind, attrs = {}) {
  ...
  if (kind === 'school-work') {
    ...
    const spotifyRedirectUri = `${window.location.origin}/api/integrations/spotify/callback`;
    let playlists = [];
              ...
              ` : `
                <div id="spotify-config-box" style="background:#fff;border:1px solid #e2e4dc;border-radius:12px;padding:16px">
                  ...
                  <div style="background:#f7f7f4;border-radius:8px;padding:10px;margin-top:14px;font-size:11px;color:#555;line-height:1.6">
                    ...
                    3. Under <strong>Redirect URIs</strong>, add: <code style="background:#eaeaea;padding:2px 5px;border-radius:4px;user-select:all">${spotifyRedirectUri}</code><br/>
...
document.addEventListener('click', async e => {
  ...
  try {
    ...
    // Backup & Persistence Actions
    if (b.id === 'btn-download-backup') {
      toast('Generating study backup...');
      try {
        const data = await api('/api/sync/backup');
        const blob = new Blob([JSON.stringify(data, null, 2)], { type: 'application/json' });
        const url = URL.createObjectURL(blob);
        const a = document.createElement('a');
        a.href = url;
        a.download = `class11_study_backup_${new Date().toISOString().slice(0, 10)}.json`;
        document.body.appendChild(a);
        a.click();
        a.remove();
        URL.revokeObjectURL(url);
        toast('Backup file downloaded successfully!');
      } catch (err) {
        toast('Failed to download backup: ' + err.message);
      }
      return;
    }

    if (b.id === 'btn-trigger-upload-backup') {
      $('#input-upload-backup')?.click();
      return;
    }
    ...
    if (b.id === 'btn-connect-spotify') {
      try {
        const curRedirect = `${window.location.origin}/api/integrations/spotify/callback`;
        const res = await api(`/api/integrations/spotify/auth-url?redirect_uri=${encodeURIComponent(curRedirect)}`);
        if (res.url) {
    ...
    if (b.id === 'btn-save-spotify-creds') {
      ...
      try {
        ...
        const res = await api('/api/integrations/spotify/configure', {
          method: 'POST',
          body: JSON.stringify({ clientId, clientSecret, redirectUri: curRedirect })
        });
...
document.addEventListener('change', async e => {
  ...
  if (e.target.id === 'input-upload-backup') {
    const file = e.target.files?.[0];
    if (!file) return;
    try {
      toast('Restoring study data from backup...');
      ...
      if (res?.ok) {
        ...
        await refresh();
        renderSettings();
        toast('✦ Full study data restored successfully!');
      }
    } catch (err) {
...
// App Bootstrap & Spotify Polling (With Seamless Auto-Persistence Restore)
(async () => {
  try {
    boot = await api('/api/bootstrap');
    const localBackup = getLocalBackup();

    // If server container woke up fresh/empty while browser has saved data, automatically restore
    const isServerEmpty = !boot.configured || 
      (boot.tasks?.length === 0 && (localBackup?.tasks?.length > 0 || localBackup?.school_work?.length > 0));

    if (isServerEmpty && localBackup) {
      console.log('Restoring saved student state from local backup...');
      const restored = await restoreFromLocalBackup(localBackup);
      if (restored) {
        boot = await api('/api/bootstrap');
      }
    }
